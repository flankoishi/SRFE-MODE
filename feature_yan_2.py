...
# 传入一张图片并获取卷积特征图
img_tensor = preprocess_image("your_image_path.jpg")  # 这里填入你的图片路径
...
# 保存特征图到本地
def save_feature_map(feature_map, layer_name, save_dir='feature_maps'):
    # 为每个层创建文件夹
...
# 保存每个卷积层的特征图
def save_all_feature_maps(feature_maps, save_dir='feature_maps'):
    for layer, feature_map in feature_maps.items():
        layer_name = str(layer)  # 获取层名称作为文件夹名称
